...
int SynchConsole::ReadFromConsoleV(int dataVaddr, int size) {
    ...
    for (k=0;k<size;k++) {
        ...
        if(!isvalid) {
            //Here double call for handling the page fault cases.
            isvalid=machine->WriteMem(dataVaddr+k,1,ch);
            if(!isvalid) {
                lock->Release();
...
void SynchConsole::WriteToConsoleV(int dataVaddr, int size) {
    ...
    for(k=0;k<tmposize;k++) {
        ...
        if (!isValid){
            isValid = machine->ReadMem(dataVaddr + k, 1, (int *) &ch[k]);
            if (!isValid){
